...
open class RingLayer : CALayer {
    ...
    //:- Public API
    open var ringWidth: CGFloat = 40.0 {
        didSet {
    ...
    open var ringColors: (CGColor, CGColor) = (UIColor.red.cgColor, UIColor.red.darkerColor.cgColor) {
        didSet {
            gradientLayer.colors = ringColors
            ringTipLayer.color = ringColors.0
        }
    }
    open var ringBackgroundColor: CGColor = UIColor.darkGray.cgColor {
        didSet {
...
extension RingLayer {
    ...
    func start(){
        ...
        ringTipLayer.add(rotationInfiniteForLayer(ringTipLayer), forKey: "transform.rotation.z")
        gradientLayer.add(rotationInfiniteForLayer(gradientLayer), forKey: "transform.rotation.z")
        CATransaction.commit()
    }
    func stop(){
        stopRotation(layer: gradientLayer)
        stopRotation(layer: ringTipLayer)
        
    }
    func stopRotation(layer : CALayer)
    {
        let currentPresentationLayer = layer.presentation()
        let currentDegree = currentPresentationLayer?.value(forKeyPath: "transform.rotation.z") as! CGFloat
        layer.removeAnimation(forKey: "transform.rotation.z")
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        layer.setValue(currentDegree, forKeyPath: "transform.rotation.z")
        CATransaction.commit()
        
    }
    func rotationInfiniteForLayer(_ layer: CALayer) -> CABasicAnimation {
        let rotationAnimation = CABasicAnimation(keyPath: "transform.rotation.z")
        
        rotationAnimation.fromValue =  angleOffsetForZero
        rotationAnimation.toValue = CGFloat(M_PI * 2) + angleOffsetForZero
